...
console.log(cartItem)
        ...
        if(cart.insertBefore(cartItem, total)){
            ...
            var deleteItem = document.querySelectorAll(".cart-item-remove");
            deleteItem.forEach(function(clear){
                clear.addEventListener('click', function(e){
                    ...
                    items.forEach(function(item){
                        totalClear.push(parseFloat(item.textContent));
                       
                    });
                    const totalMoneyclear = totalClear.reduce(function(total, item){
                        total += item;
                        return total;
                    },0);
                    const finalMoneyClear = totalMoneyclear.toFixed(2);
                    document.getElementById('cart-total').textContent = finalMoneyClear;
                    document.querySelector('.item-total').textContent = finalMoneyClear;
                    document.getElementById('item-count').textContent = totalClear.length;
                });
...
function showTotals(){
    ...
    console.log(typeof total.length)
